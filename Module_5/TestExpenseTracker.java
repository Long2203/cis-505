...
public class TestExpenseTracker {
	...
	public static void main(String[] args) {
	    ...
	    // initialize the continueChoice boolean variable as true
	    boolean continueChoice = true;
	    
	    // create a while loop that will display the menu options and receive the user's input until continueChoice is false
	    while (continueChoice){
			...
			switch(choice) {
	            case 1:
	                viewTransactions(); // call the viewTransactions() method
	                break;
	            case 2:
	            	String c = "y";
	            	while (c.equalsIgnoreCase("y"))
	            	 {		addTransaction(); // call the addTransaction() method
	            		 c = ValidatorIO.getString(input,"\n  Add another transaction? (y/n): "); // prompt the user to add another transaction
	                 } 
	                break;
	            case 3:
	                viewExpense(); // call the viewExpense() method
	                break;
	            default:
	        ...
	        if (continueInput.equalsIgnoreCase("n")) {
	            continueChoice = false; // set continueChoice to false to exit the while loop
	            System.out.println("\n  Program terminated by the user..."); // display a message that the program is terminated by the user
	        }
